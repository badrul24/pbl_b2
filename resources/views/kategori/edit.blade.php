...
@section('content')
    ...
    <div class="flex flex-col gap-4 md:flex-row md:items-center md:justify-between mb-6">
        <div>
            ...
            <p class="mt-1 text-sm text-gray-500">Form Edit Data Kategori</p>
        </div>
            ...
            <form action="{{ route('kategori.update', $kategori->id) }}" method="POST" class="space-y-4">
                ...
                <div>
                    ...
                    <textarea name="deskripsi" rows="3"
                        class="w-full border border-gray-300 rounded-md px-3 py-2 focus:outline-none focus:ring-2 focus:ring-primary-500 focus:border-primary-500 @error('deskripsi') border-red-500 @enderror">{{ old('deskripsi', $kategori->deskripsi) }}</textarea>
                    @error('deskripsi')
                        <p class="text-red-500 text-sm mt-1">{{ $message }}</p>
                    @enderror
                </div>
