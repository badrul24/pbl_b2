...
@section('content')
    ...
    <div class="flex flex-col gap-4 md:flex-row md:items-center md:justify-between mb-6">
        <div>
            <h2 class="text-2xl font-bold text-gray-900">Edit Kode Klasifikasi</h2>
            <p class="mt-1 text-sm text-gray-500">Form Edit Data Kode Klasifikasi</p>
        </div>
    ...
    <div class="bg-white rounded-lg shadow-sm p-6">
        <div class="max-w-xl">
            <form action="{{ route('kode.update', $kode->id) }}" method="POST" class="space-y-4">
                @csrf
                @method('PUT')
                <div>
                ...
                <div>
                    ...
                    <input type="text" name="nama_kode"
                        ...
                        value="{{ old('nama_kode', $kode->nama_kode) }}">
                    @error('nama_kode')
                        <p class="text-red-500 text-sm mt-1">{{ $message }}</p>
                    @enderror
                </div>
                ...
                <div>
                    ...
                    <select name="kategori_id"
                        ...
                        @foreach($kategoris as $kategori)
                            <option value="{{ $kategori->id }}" {{ (old('kategori_id', $kode->kategori_id) == $kategori->id) ? 'selected' : '' }}>
                ...
                <div>
                    ...
                    <textarea name="keterangan" rows="3"
                        class="w-full border border-gray-300 rounded-md px-3 py-2 focus:outline-none focus:ring-2 focus:ring-primary-500 focus:border-primary-500 whitespace-normal break-words resize-none @error('keterangan') border-red-500 @enderror">{{ old('keterangan', $kode->keterangan) }}</textarea>
                    @error('keterangan')
                        <p class="text-red-500 text-sm mt-1">{{ $message }}</p>
                    @enderror
                </div>
